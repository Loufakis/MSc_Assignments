...
public class WallJumpAgent : Agent
{
    ...
    // Check if there is a high wall
    bool highWall = false;
    bool availableWallReward = true;
...

    {
        ...
        if (highWall == true)
        {
            if (shortBlock.GetComponent<Collider>().gameObject.tag == "wall" && availableWallReward == true)
            {
                SetReward(+0.5f);
                availableWallReward = false;
                Debug.Log("Box to the wall!");
            }
        }
    }
